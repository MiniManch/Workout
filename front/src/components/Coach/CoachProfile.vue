<template>
    <div class="containerOfAll">
      ...
      <YesOrNoModal v-if="showYesNoModal" :title="modalTitle" :message="modalMessage" @close="handleModalClose"/>
    ...
    export default {
        ...
        methods: {
            ...
            confirmDeleteClient({itemId }) {
                ...
                this.modalMessage = 'Are you sure you want to delete this client? PENIS VAGINA';
                this.itemToDelete = itemId;
                this.showYesNoModal = true;

                console.log(this.showYesNoModal)
            },

            async performAction() {
                await this.deleteClient({ type: 'client', itemId: this.itemToDelete });
                this.showYesNoModal = false;
                this.itemToDelete = null;
            },

            async deleteClient({ type, itemId }) {
            try {
                const response = await fetch(`/api/${type}/delete/${itemId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
